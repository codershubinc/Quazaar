...
- **Method:** `GET` (Note: The web test used `GET`, ensure consistency with backend) or `POST` depending on server implementation. _Correction based on web test:_ The web test uses `GET`.
- **Response:** JSON
  ```json
  {
